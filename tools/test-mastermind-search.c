#include <ctype.h>
#include <math.h>
#include <stdbool.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

#define SANTRON_VM_NO_MAIN
#include "santron-vm.c"

static bool load_listing_file(SantronState *s, const char *path) {
  FILE *fp = fopen(path, "r");
  if (!fp) {
    perror(path);
    return false;
  }

  char line[256];
  while (fgets(line, sizeof(line), fp)) {
    char *p = line;
    while (isspace((unsigned char)*p)) p++;
    if (*p == '\0' || *p == '#' || *p == ';') continue;

    int addr = -1;
    int code = -1;
    if (sscanf(p, "%d %d", &addr, &code) == 2) {
      if (addr >= 0 && addr < PROGRAM_SIZE) s->program[addr] = code;
    }
  }

  fclose(fp);
  s->pc = 0;
  return true;
}

static bool find_pause_pcs(const SantronState *s, int *input_pc, int *result_pc) {
  int first = -1;
  int last = -1;

  for (int i = 0; i < PROGRAM_SIZE; i++) {
    if (s->program[i] == KEY_RS) {
      if (first < 0) first = i;
      last = i;
    }
  }

  if (first < 0 || last < 0 || first == last) return false;
  *input_pc = first + 1;
  *result_pc = last + 1;
  return true;
}

static bool has_duplicate_digits(const int digits[4]) {
  for (int i = 0; i < 4; i++) {
    for (int j = i + 1; j < 4; j++) {
      if (digits[i] == digits[j]) return true;
    }
  }
  return false;
}

static int encode_code(const int code[4]) {
  static const int pow10[] = {
    1, 10, 100, 1000, 10000, 100000, 1000000
  };
  int encoded = 0;
  for (int pos = 0; pos < 4; pos++) {
    encoded += (pos + 1) * pow10[code[pos]];
  }
  return encoded;
}

static void expected_score_unique_code(const int code[4], const int guess[4], int *black, int *white) {
  int hits = 0;
  *black = 0;

  for (int i = 0; i < 4; i++) {
    if (code[i] == guess[i]) (*black)++;
    for (int j = 0; j < 4; j++) {
      if (guess[i] == code[j]) hits++;
    }
  }

  *white = hits - *black;
}

static bool close_to(double actual, int expected) {
  return fabs(actual - (double)expected) <= 1e-8;
}

static void press_memory_key(SantronState *s, int command, int digit) {
  execute_key(s, command, false);
  execute_key(s, KEY_DIGIT0 + digit, false);
}

static void print_digits(const int digits[4]) {
  for (int i = 0; i < 4; i++) printf("%d", digits[i]);
}

static bool run_one_case(const SantronState *base, int input_pause_pc, int result_pause_pc,
                         const int code[4], const int guess[4]) {
  int expected_black = 0;
  int expected_white = 0;
  expected_score_unique_code(code, guess, &expected_black, &expected_white);

  SantronState s = *base;
  s.memory[1] = 0.0;
  s.memory[8] = (double)encode_code(code);
  s.memory[9] = 4.0;

  run_program(&s, 1000);
  for (int i = 0; i < 4; i++) {
    execute_key(&s, KEY_DIGIT0 + guess[i], false);
    run_program(&s, 1000);
  }

  bool ok = true;
  ok = ok && s.paused && s.pc == result_pause_pc;
  ok = ok && close_to(s.memory[0], 1);
  ok = ok && close_to(s.memory[1], expected_black);
  ok = ok && close_to(s.memory[2], 5);
  ok = ok && close_to(s.memory[9], expected_white);

  press_memory_key(&s, KEY_RCL, 1);
  ok = ok && close_to(s.x, expected_black);
  press_memory_key(&s, KEY_RCL, 9);
  ok = ok && close_to(s.x, expected_white);
  execute_key(&s, KEY_DIGIT0 + 0, false);
  press_memory_key(&s, KEY_STO, 1);
  execute_key(&s, KEY_DIGIT0 + 4, false);
  press_memory_key(&s, KEY_STO, 9);

  run_program(&s, 1000);
  ok = ok && s.paused && s.pc == input_pause_pc;
  ok = ok && close_to(s.memory[0], 1);
  ok = ok && close_to(s.memory[1], 0);
  ok = ok && close_to(s.memory[2], 1);
  ok = ok && close_to(s.memory[9], 4);

  if (!ok) {
    printf("FAIL code=");
    print_digits(code);
    printf(" guess=");
    print_digits(guess);
    printf(" expected=%d/%d got x=%.12g M1=%.12g M2=%.12g M9=%.12g pc=%d paused=%d\n",
           expected_black, expected_white, s.x, s.memory[1], s.memory[2],
           s.memory[9], s.pc, s.paused ? 1 : 0);
  }

  return ok;
}

int main(int argc, char **argv) {
  const char *program_path = "mastermind-search.lst";
  bool preset_m0_one = false;

  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--m0-one") == 0) {
      preset_m0_one = true;
    } else if (argv[i][0] == '-') {
      fprintf(stderr, "usage: %s [--m0-one] [listing.lst]\n", argv[0]);
      return 2;
    } else {
      program_path = argv[i];
    }
  }

  SantronState base;
  init_state(&base);
  if (preset_m0_one) base.memory[0] = 1.0;
  if (!load_listing_file(&base, program_path)) return 2;

  int input_pause_pc = -1;
  int result_pause_pc = -1;
  if (!find_pause_pcs(&base, &input_pause_pc, &result_pause_pc)) {
    fprintf(stderr, "cannot find input/result R/S instructions in %s\n", program_path);
    return 2;
  }

  int tested = 0;

  for (int c0 = 1; c0 <= 6; c0++) {
    for (int c1 = 1; c1 <= 6; c1++) {
      for (int c2 = 1; c2 <= 6; c2++) {
        for (int c3 = 1; c3 <= 6; c3++) {
          int code[4] = {c0, c1, c2, c3};
          if (has_duplicate_digits(code)) continue;

          for (int g0 = 1; g0 <= 6; g0++) {
            for (int g1 = 1; g1 <= 6; g1++) {
              for (int g2 = 1; g2 <= 6; g2++) {
                for (int g3 = 1; g3 <= 6; g3++) {
                  int guess[4] = {g0, g1, g2, g3};
                  if (has_duplicate_digits(guess)) continue;

                  if (!run_one_case(&base, input_pause_pc, result_pause_pc, code, guess)) {
                    return 1;
                  }

                  tested++;
                }
              }
            }
          }
        }
      }
    }
  }

  printf("PASS %d code/guess combinations; input halt pc=%d result halt pc=%d\n",
         tested, input_pause_pc, result_pause_pc);
  return 0;
}
